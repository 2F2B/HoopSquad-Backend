...
function LoginKakao(code) {
  return __awaiter(this, void 0, void 0, function* () {
    const token = yield axios_1.default.post(
    ...
    const user = yield axios_1.default.get(
    ...
    const refresh_token_expires_in = token.data.refresh_token_expires_in;
    if (!isUserExist) {
      //유저 정보가 DB에 없음
    ...
    if (request.body.access_token) {
      ...
      if (tokenResult) {
        if (
          ...
          const newToken = yield axios_1.default.post(
            "https://kauth.kakao.com/oauth/token",
            {
